...
puts " TEST 1 : INSERT "
MySqliteRequest.new.insert("users").values({"name" => "Alice", "age" => 20}).run
MySqliteRequest.new.insert("users").values({"name" => "Bob", "age" => 25}).run
...
MySqliteRequest.new.update("users").set({"name" => "Robert"}).where("name", "Bob").run
